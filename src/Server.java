// essentially the same as the I.S.S's server, but makes a new thread per requests in main
import java.io.*;
import java.net.*;
import java.time.*;
import java.util.*;
...
public class Server {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) { //keep server running 
            ...
            new Thread(() -> { // new thread for each request
                try {
                    handleClient(clientSocket); //perform the operation requested & return the data
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }).start();
